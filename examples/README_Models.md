...
针对风机 SCADA 数据（时序、多变量、非线性）的异常检测模型推荐。建议结合**滑动窗口**处理时序特征。

| 学习范式 | 模型 | 推荐度 | 典型应用 | 理由 |
| :--- | :--- | :--- | :--- | :--- |
| **无监督 (Unsupervised)** <br> *无需标签，最适用* | **IForest** (Isolation Forest) | ⭐⭐⭐⭐⭐ | 早期预警、数据清洗 | 工业界标准 Baseline，鲁棒性极强，不怕噪声，速度快。 |
| | **AutoEncoder / VAE** | ⭐⭐⭐⭐⭐ | 功率曲线建模、部件老化监测 | 擅长捕捉风速-功率的非线性关系，重构误差能敏锐发现偏离曲线的异常。 |
| | **KNN (Mahalanobis)** | ⭐⭐⭐⭐ | 功率曲线离群点检测 | 考虑变量相关性（协方差），能有效区分“高风速低功率”等违反物理规律的异常。 |
| | **ECOD / COPOD** | ⭐⭐⭐⭐ | 实时流式检测 | 无需训练参数，推理极快，适合算力受限的边缘端部署。 |
| | **LOF / CBLOF** | ⭐⭐⭐ | 局部异常检测 | 适合发现密度分布不均的局部异常，但计算复杂度较高。 |
| **半监督 (Semi-supervised)** <br> *仅有正常数据或少量异常* | **DeepSVDD** | ⭐⭐⭐⭐ | 单一工况建模 | 假设训练数据绝大多数为正常，通过最小化超球体体积来包裹正常样本。 |
| | **XGBOD** | ⭐⭐⭐⭐ | 历史故障库利用 | 利用无监督模型的输出作为特征，结合少量已知故障标签进行增强训练。 |
| | **OCSVM** | ⭐⭐⭐ | 小样本建模 | 经典的单分类方法，适合数据量较小且纯净的场景。 |
| **监督 (Supervised)** <br> *标签完备* | **XGBoost / LightGBM** | ⭐⭐⭐⭐⭐ | 故障分类/诊断 | *注：非PyOD内置算法(XGBOD除外)*。若有完备标签，监督学习分类器通常优于OD算法。 |
